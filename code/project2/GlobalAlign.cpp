...
//Returen squared Euclidean distance
double DistSquared(const Feature k1, const Feature k2)
{
    double dif, distsq = 0;

	for (int i = 0; i < 128; i++)
	{
		dif = k1.data[i] - k2.data[i];
		distsq += dif*dif;
	}
    return distsq;
}

// search on f2 to find the matching feature with key feature with threshold.
int matchOneFeature( const Feature key, const FeatureSet &f2, FeatureMatch &match)
{
	double distsq, distsq1 = 1e10, distsq2 = 1e10;
	int id1 = -1;
	double threshold = 0.6f;

	for (unsigned int i = 0; i < f2.size(); i++)
	{
		distsq = DistSquared(key,f2[i]);
		if (distsq < distsq1) 
		{
			distsq2 = distsq1;
			distsq1 = distsq;
			id1 = f2[i].id;
			//printf("%d, %d, ",i,id1); //id = i + 1
		} 
	...
	if (distsq1 < threshold*threshold*distsq2)
	{
		match.id = id1;
		match.score = sqrt(distsq1);
		//printf("%d-%d, ",key.id,match.id);  
	}
	else
		match.id = -1;
...
int matchFeatures(const FeatureSet &f1, const FeatureSet &f2, vector<FeatureMatch> &matches)
{
	...
	for (unsigned int i = 0; i < f1.size(); i++)
	{
		...
		matchOneFeature( f1[i], f2, match);
		
		matches.push_back(match);
		//if (match.id != -1)
		//	printf("Find a match: %d.\n", match.id);
	}
	return 0;
}
	...
	for (int i=0; i<n; i++) {
		for (int j=0; j<n; j++) {
			if (i != j) {
				...
				matchFeatures(fs[i], fs[j], am[i][j].matches);
				
				alignImagePair(fs[i], fs[j], am[i][j].matches, m, f, width, height, nRANSAC, RANSACthresh, am[i][j].r, am[i][j].inliers);
				//printf("\n%d %d \n", width, height);
				//printf("\n%f %f %f\n", am[i][j].r[0][0], am[i][j].r[0][1], am[i][j].r[0][2]);
				//printf("%f %f %f\n", am[i][j].r[1][0], am[i][j].r[1][1], am[i][j].r[1][2]);
				//printf("%f %f %f\n", am[i][j].r[2][0], am[i][j].r[2][1], am[i][j].r[2][2]);
				// END TODO
